...
var Chat = {
	...
	enable_queue: false, queue_text: null,
	...
	buildLogin: function (element) {
		if (!Chat.guestName) {
			...
			if (element.childNodes.length === 0) {
				...
				chat_close.innerHTML = '<button type=\"submit\" class=\"dolphin-message-close\">Close</button>';

				element.innerHTML = element.innerHTML + '<label class=\"dolphin-connect-message\">' + Chat.connect_message + "</label>";
				element.innerHTML = element.innerHTML + '<label id=\"warning-login\" class=\"dolphin-warning-login\"></label>';
				element.innerHTML = element.innerHTML + '<input id=\"name\" type=\"text\" class=\"dolphin-name-input\" placeholder=\"Name\"></input><br>';
				element.innerHTML = element.innerHTML + '<label id=\"warning-name\" class=\"dolphin-warning-name\"></label>';
				element.innerHTML = element.innerHTML + '<br><br><input id=\"email\" type=\"text\" class=\"dolphin-email-input\" placeholder=\"Email\"></input><br>';
				element.innerHTML = element.innerHTML + '<label id=\"warning-email\" class=\"dolphin-warning-email\"></label>';
				element.innerHTML = element.innerHTML + '<br><br><input id=\"telephone\" type=\"text\" class=\"dolphin-phone-input\" placeholder=\"Phone\"></input><br>';
				element.innerHTML = element.innerHTML + '<label id=\"warning-phone\" class=\"dolphin-warning-phone\"></label>';
				if (Chat.g_captcha_key !== null && Chat.g_captcha_key !== undefined) {
					element.innerHTML = element.innerHTML + "<div class=\"captcha\"><div class=\"dolphin-g-recaptcha\" data-sitekey=\"" + Chat.g_captcha_key + "\" data-theme=\"dark\"></div></div>";
				} else {
					element.innerHTML = element.innerHTML + "<br><br>";
				}
				element.innerHTML = element.innerHTML + '<button type=\"submit\" class=\"dolphin-login-connect\">Connect</button>';
	...
	buildChat: function (element, skip) {
		if (element.childNodes.length === 0) {
			...
			messages_box.innerHTML = '<textarea type=\"text\" class=\"message-input\" placeholder=\"' + Chat.type_placeholder + '\"></textarea>' + '<form method="POST" enctype="multipart/form-data" id="attachment-form"><input id="attachment" type=\"file\" class=\"dolphin-inputfile inputfile-1\" onchange=\"uploadFile()\"/>' + '<label for="attachment"><figure class="dolphin-attachment-figure"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 17"><path d="M10 0l-5.2 4.9h3.3v5.1h3.8v-5.1h3.3l-5.2-4.9zm9.3 11.5l-3.2-2.1h-2l3.4 2.6h-3.5c-.1 0-.2.1-.2.1l-.8 2.3h-6l-.8-2.2c-.1-.1-.1-.2-.2-.2h-3.6l3.4-2.6h-2l-3.2 2.1c-.4.3-.7 1-.6 1.5l.6 3.1c.1.5.7.9 1.2.9h16.3c.6 0 1.1-.4 1.3-.9l.6-3.1c.1-.5-.2-1.2-.7-1.5z"/></svg></figure></label></form>' + '<button type=\"submit\" class=\"message-submit\">Send</button>';
		...
		$(document).ready(function () {
			new Blazy({
				container: '.dolphin-messages .messages-content',
				offset: 100
			});
		})
	},
	buildError: function (element, message) {
		...
		if (element !== null) {
			...
			$(".dolphin-message-box .message-input").attr('disabled', 'disabled');
			if (message.toLowerCase().indexOf("connection") !== -1 && localStorage.getItem("messages") !== null) {
				...
				$(".login").hide();
				$('<div class=\"dolphin-error-login-box\"><label id=\"error-login\" class=\"dolphin-error-login\">"' + message + '"</label></div>').appendTo(element);
			}
		}
		Chat.isconnecting = false;
	},
...
function sendTextMessage(msgObj, message) {
	...
	$('<div id="' + tx.id + '" class="message message-sending">' + message + '</div>').appendTo($('.dolphin-messages .messages-content')).addClass('new');
	transactions.push({ message: message, txId: transactionId });
	tx.commit();
}
...
function renderButton(buttons) {
	...
	buttons.forEach(function (button) {
		...
		if (typeof button.pictureLink !== "undefined") {
			buttonImage = buttonImage + '<img class="dolphin-btn-card__image b-lazy" src="https://cdn.3dolphins.ai:9443/webchat/images/ic_loading.gif" data-src="' + button.pictureLink + '" onerror="this.src=https://cdn.3dolphins.ai:9443/webchat/images/ic_image_empty.jpg"></img>';
		}
	...
	$(completedButton).appendTo($('.dolphin-messages .messages-content')).addClass('new');
	if (buttons.length > 1) {
...
function viewForm(url) {
	$('<iframe src="' + url + '&overflowY=scroll" frameBorder="0" width="320" height="410"/>').appendTo('#dolphin-messages');
...
function viewPage(url) {
	$('<iframe src="' + url + '&overflowY=scroll" frameBorder="0" width="100%" height="380"/>').appendTo('#dolphin-messages');
	$('#messages-content').css("visibility", "hidden");
	$('.dolphin-message-box .message-input').css("visibility", "hidden");
	$('.dolphin-message-box form label').css("visibility", "hidden");
	$('.dolphin-message-box .dolphin-message-box .message-submit').css("visibility", "hidden");
